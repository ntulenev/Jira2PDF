...
internal sealed class JiraTransport : IJiraTransport
{
    ...
    public async Task<TDto?> GetAsync<TDto>(Uri url, CancellationToken cancellationToken)
        => await SendAsync<TDto>(url, static requestUrl => new HttpRequestMessage(HttpMethod.Get, requestUrl), cancellationToken).ConfigureAwait(false);

    /// <inheritdoc />
    public async Task<TDto?> PostAsync<TRequest, TDto>(Uri url, TRequest request, CancellationToken cancellationToken)
    {
        ...
        return await SendAsync<TDto>(
            url,
            requestUrl => new HttpRequestMessage(HttpMethod.Post, requestUrl)
            {
